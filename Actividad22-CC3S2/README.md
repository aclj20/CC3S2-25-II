...
1. Verifique que mi máquina tiene:

![alt text](image.png)

2. Crea y activa el entorno `bdd` en la raíz del proyecto:Instala dependencias:
...
Desde `Observabilidad-mcp/` ejecute el comando `make up`, esto levanta los servicios `app`, `otel-collector`, `prometheus`, `loki`, `promtail`, `tempo`, `grafana`, `mcp-server`
